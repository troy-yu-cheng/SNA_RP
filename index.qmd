...
The results of the LRT show that the $p$-value of the $\chi^2$ test is very close to 0, indicating a significant difference in the goodness of fit between the two models. Therefore, we can confidently conclude that the relationship between the distance from the province boundary and the density of memorial facilities differs between the inside and outside of the province. Therefore, the null hypothesis is rejected and **H4** is supported.
...
In the 1000 samples, the points inside the province consistently exhibited a Poisson point process distinct from the points outside the province, and the difference in model fit was **highly significant**, with the largest $p$-value extremely close to 0. Therefore, we can be highly confident in rejecting the null hypothesis of **H4**.
